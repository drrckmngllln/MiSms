...
class DiscountHelper
{
    ...
    public static function processDiscount($request, $studentAssessment, $discount_id)
    {
        ...
        if ($studentAssessment) {
            ...
            if (!is_numeric($discountPercentage) || $discountPercentage < 0 || $discountPercentage > 2000) {
            ...
            if ($discountPercentage == 100 && $discountTarget == 'Tuition Fee 100%') {
                ...
                $studentAssessment->discountCompute = $studentAssessment->computation2Tuition;
                $newTotalAss = $studentAssessment->total_assessment -= $studentAssessment->computation2Tuition;
                ...
                $newTotalAss = $studentAssessment->total_assessment - $studentAssessment->computation - $studentAssessment->total_miscfee_first_year;

                $studentAssessment->computation2Tuition = 0;
                $studentAssessment->total_miscfee_first_year = 0;
                $studentAssessment->total_assessment = $newTotalAss;
                $discount = $studentAssessment->discountCompute = $studentAssessment->tutionFeesDeleteSub;
                ...
                fee_summary::create([
                    'id_number' => $request->id_number,
                    'date' => Carbon::now()->format('Y-m-d'),
                    'downpayment' =>  $studentAssessment->discountCompute + ($studentAssessment->discountComputeMiscFee ?? 0),
                    'or_number' => $request->or_number,
                    'particulars' => $discountTarget,
                    'total_assessment' => $studentAssessment->total_assessment,
                ...
                $totalDiscount = 0.35 * $studentAssessment->computation2Tuition;
                $studentAssessment->discountCompute = $totalDiscount;
                $newTotalAss =  $studentAssessment->total_assessment -= $totalDiscount;
                $studentAssessment->computation2Tuition -= $totalDiscount;
                ...
                $tuitionDiscount = min($totalDiscount, $studentAssessment->computation2Tuition);
                $studentAssessment->computation2Tuition -= $tuitionDiscount;
                $remainingDiscount = $totalDiscount - $tuitionDiscount;
                ...
                if ($remainingDiscount > 0) {
                    studentAssesment::where('id_number', $studentAssessment->id_number)
                        ...
                        ->where('category', '!=', 'Tuition Fee')
                        ->orderBy('category')
                        ->each(function ($assessment) use (&$remainingDiscount, &$transferredCategories) {
                            if ($remainingDiscount > 0) {

                                $transferAmount = min($remainingDiscount, $assessment->computation2Tuition);
                                $assessment->computation2Tuition -= $transferAmount;
                ...
                foreach ($transferredCategories as $transferred) {
                    DB::table('fee_collection_histories')->insert([
                        'id_number' => $studentAssessment->id_number,
                        'category' => $transferred['category'],
                        'amount_subtracted' => $transferred['transferred_amount'],
                        'created_at' => Carbon::now()->format('Y-m-d'),
                        'subCategory' => $studentAssessment->discount?->code,
                        'or_number' => $request->or_number,
                    ]);
                }
                $discount = $studentAssessment->discountCompute = $studentAssessment->tutionFeesDeleteSub;
                ...
                $totalDiscount = 0.05 * $studentAssessment->computation;
                $studentAssessment->discountCompute = $totalDiscount;
                $newTotalAss =  $studentAssessment->total_assessment -= $totalDiscount;
                ...
                $totalDiscount = 0.10 * $studentAssessment->computation;
                $studentAssessment->discountCompute = $totalDiscount;
                $newTotalAss =  $studentAssessment->total_assessment -= $totalDiscount;
                $studentAssessment->computation -= $totalDiscount;
                ...
                $studentAssessment->computation = 0;
            ...
            } elseif ($discountPercentage == 25 && $discountTarget == '25% Discount on Tuition Fee SK CHAIRMAN') {
                $totalDiscount = 0.25 * $studentAssessment->computation;
                $studentAssessment->discountCompute = $totalDiscount;
                $newTotalAss = $studentAssessment->total_assessment -= $totalDiscount;
                $studentAssessment->computation -= $totalDiscount;
                $discount = $studentAssessment->discountCompute = $studentAssessment->tutionFeesDeleteSub;
            ...
            } elseif ($discountPercentage == 25 && $discountTarget == '25% Discount on Tuition Fee SONS/DAUGTHERS OF BRGY.CAPTAIN') {
                $totalDiscount = 0.25 * $studentAssessment->computation;
                $studentAssessment->discountCompute = $totalDiscount;
                $newTotalAss = $studentAssessment->total_assessment -= $totalDiscount;
                $studentAssessment->computation -= $totalDiscount;
                ...
                $activityLog = [
                    ...
                    'email' => $user->email,
                    'role_name' => $roleName,
                    'modify_user' => 'Apply Discount 25% Discount on Tuition Fee SONS/DAUGTHERS OF BRGY.CAPTAIN to Student ID' . $studentAssessment->id_number,
                    'date_time' => $dt->format('D, M j, Y g:i A'),
                ];
                ...
                $totalDiscount = 2000;
                $totalDiscount = min($totalDiscount, $studentAssessment->total_assessment);
                $studentAssessment->discountCompute = $totalDiscount;
                $newTotalAss = $studentAssessment->total_assessment -= $totalDiscount;
                $studentAssessment->computation -= $totalDiscount;
                $discount = $studentAssessment->discountCompute = $studentAssessment->tutionFeesDeleteSub;
            ...
            if ($discountPercentage == 100 && $discountTarget == '100% MCNP-ISAP SCHOLAR/GRANTEES') {
                ...
                studentAssesment::where('id_number', $studentAssessment->id_number)
                    ->where('semester', $latestSemester)
                    ->orderBy('semester', 'desc')
                    ->update(array_fill_keys($propertiesToSetToZero, 0));
                fee_summary::create([
                    'id_number' => $request->id_number,
                    'date' => Carbon::now()->format('Y-m-d'),
                    'downpayment' => $studentAssessment->discountCompute,
