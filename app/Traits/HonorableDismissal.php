...
trait HonorableDismissal
{


    //add method
    //nag lagay lang ako ng variable $inputName tas variable $path
    public function uploadHD(Request $request, $inputName, $path)
    {
        ...
        if ($request->hasFile($inputName)) {
            ...
            $imageName = 'StudentHonorableDismissal_' . uniqid() . '.' . $ext;
            $image->move(public_path($path), $imageName);

            return $path . '/' . $imageName;
        }
    }
    public function deleteHD(string $path)
    {
