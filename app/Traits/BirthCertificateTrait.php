...
trait BirthCertificateTrait
{


    //add method
    //nag lagay lang ako ng variable $inputName tas variable $path
    public function uploadBirthCertificate(Request $request, $inputName, $path)
    {
        ...
        if ($request->hasFile($inputName)) {
            ...
            $imageName = 'StudentBirthCertificate_' . uniqid() . '.' . $ext;
            $image->move(public_path($path), $imageName);

            return $path . '/' . $imageName;
        }
    }
    public function deleteuploadBirthCertificate(string $path)
    {
