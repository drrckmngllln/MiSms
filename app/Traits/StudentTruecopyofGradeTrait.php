...
trait StudentTruecopyofGradeTrait
{


    //add method
    //nag lagay lang ako ng variable $inputName tas variable $path
    public function uploadTrueCopyofGrades(Request $request, $inputName, $path)
    {
        ...
        if ($request->hasFile($inputName)) {
            ...
            $imageName = 'StudentTrueCopyofGrades_' . uniqid() . '.' . $ext;
            $image->move(public_path($path), $imageName);

            return $path . '/' . $imageName;
        }
    }
    public function deleteTrueCopyofGrades(string $path)
    {
