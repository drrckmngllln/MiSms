...
trait TranscriptofRecord
{


    //add method
    //nag lagay lang ako ng variable $inputName tas variable $path
    public function uploadTranscriptOfRecords(Request $request, $inputName, $path)
    {
        ...
        if ($request->hasFile($inputName)) {
            ...
            $imageName = 'StudentTOR_' . uniqid() . '.' . $ext;
            $image->move(public_path($path), $imageName);

            return $path . '/' . $imageName;
        }
    }
    public function deleteTor(string $path)
    {
