...
@push('scripts')
    <script>
        function approveStudent(id, first_name, middle_name, last_name, code, course_id) {
            ...
            $('#curriculum_id_select').on('submit', function(e) {
                ...
                var route = "{{ route('superadmin.studentapp.changeStatus', ['id' => ':id']) }}";
            ...
            $.ajax({
                type: 'GET',
                url: '{{ route('superadmin.get.CurrculumCourses') }}',
                data: {
                    course_id: course_id,
    ...
    <script>
        $(document).ready(function() {
            var createAccountTable = $('createaccount-table').DataTable({
                stateSave: true,
            });
            createAccountTable.on('page.dt', function() {
                var info = table.page.info();
                $('#current_page').val(info.page + 1);
            });
            @if (session('current_page'))
                var page = {{ session('current_page') }};
